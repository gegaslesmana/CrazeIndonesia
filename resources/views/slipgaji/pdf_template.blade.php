...
@endphp
               <img src="{{ $logo_base64 }}" alt="Logo PT.Craze Indonesia" style="width:80px;">
                <div class="company-info">
                    <div class="company-name">PT. Craze Indonesia</div>
                    <div class="company-address">
                        Pasirgombong, Cikarang Utara, Bekasi Regency, West Java 17530<br>
                        Telp: (021) 89844532
                    </div>
                </div>
            </div>
            ...
            <div class="section-body">
                <div class="row">
                    <div>Gaji Pokok</div>
                    <div class="currency">{{ number_format($d['gaji_pokok'],0,',','.') }}</div>
                </div>
                @foreach ($jenis_tunjangan as $j)
            ...
            <div class="section-body">
                @if($total_denda>0)
                    <div class="row">
                        <div>Denda</div>
                        <div class="currency">{{ number_format($total_denda,0,',','.') }}</div>
                    </div>
                @endif
                @if($jumlah_potongan_jam>0)
                    <div class="row">
                        <div>Pot. Jam ({{ number_format($total_potongan_jam,2) }})</div>
                        <div class="currency">{{ number_format($jumlah_potongan_jam,0,',','.') }}</div>
                    </div>
                @endif
                @if($d['bpjs_kesehatan']>0)
                    <div class="row">
                        <div>BPJS Kes</div>
                        <div class="currency">{{ number_format($d['bpjs_kesehatan'],0,',','.') }}</div>
                    </div>
                @endif
                @if($d['bpjs_tenagakerja']>0)
                    <div class="row">
                        <div>BPJS TK</div>
                        <div class="currency">{{ number_format($d['bpjs_tenagakerja'],0,',','.') }}</div>
                    </div>
                @endif
